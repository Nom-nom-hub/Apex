 ...
 */
export function scanRoutes(routesDir: string): Route[] {
  const routes: Route[] = [];
    ...
    for (const entry of entries) {
      ...
      if (stats.isDirectory()) {
        ...
        const routeSegment = entry.startsWith('[') && entry.endsWith(']') 
          ? entry 
          : entry;
        scanDirectory(fullPath, basePath ? join(basePath, routeSegment) : routeSegment);
      } else if (stats.isFile() && entry.endsWith('.page.tsx')) {
        // Process .page.tsx files
        const fileName = basename(entry, '.page.tsx');
        ...
        let routePath;
        if (routeSegment) {
          routePath = basePath ? join(basePath, routeSegment) : routeSegment;
        } else {
          routePath = basePath;
        }
  ...
  return routes.sort((a, b) => {
    ...
    if (a.isDynamic && b.isDynamic) {
      ...
      if (aFirstSegment.startsWith('[') && !bFirstSegment.startsWith('[')) return 1;
      if (!aFirstSegment.startsWith('[') && bFirstSegment.startsWith('[')) return -1;
    }
